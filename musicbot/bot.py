...
class MusicBot(discord.Client):
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    ...
                    'Usa %ssummon para invocarme en tu canal actual.' % self.config.command_prefix)
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        if self.user.bot:
            ...
            return Response(
                "Pushale aqui para invitarme a tu server: \n{}".format(url),
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if not info:
            raise exceptions.CommandError("No puedo poner esa we, sory.", expire_in=30)
        ...
        if 'entries' in info:
            ...
            reply_text = "Agregue **%s** a la playlist. Posicion en la lista: %s"
            btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("No puedo reproducir esa playlist we.")
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if not info:
            return Response("No encontre nada we.", delete_after=30)
        ...
        for e in info['entries']:
            result_message = await self.safe_send_message(channel, "Resultado %s/%s: %s" % (
                info['entries'].index(e) + 1, len(info['entries']), e['webpage_url']))

            confirm_message = await self.safe_send_message(channel, "Esto buscabas? Escribe `y`, `n` or `exit`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("Ok, a ver.", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response("Perfecto, ya la pongo!", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)

        return Response("Oh raios... /ff :frowning:", delete_after=30)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Reproduciendo: **%s** añadida por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "Reproduciendo: **%s** %s\n" % (player.current_entry.title, prog_str)
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('No estas en un canal de voz!')
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('Ya estoy en pausa we', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('No estoy pausado we', expire_in=30)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError("No se puede dar skip! El reproductor no tiene canciones!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("La cancion (%s) esta descargando, dame un momento we." % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    print("La siguiente cancion se reproducira en un ratito.  Espera por favor.")
                else:
                    print("Algo va mal... Houston, tenemos un problema.  "
                          "Tal vez deberias reiniciarme si no funciono bien.")
            else:
                print("Algo raro me pasa.  "
                      "Tal vez deberias reiniciarme si no funciono bien.")
        ...
        if skips_remaining <= 0:
            player.skip()  # check autopause stuff here
            return Response(
                'Le has dado skip a **{}** we.'
                '\nSkipeando...{}'.format(
                    player.current_entry.title,
                    ' Ahí viene la siguiente!' if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        try:
            ...
            raise exceptions.CommandError('{} no es un numero valido'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response(' el volumen cambio de %d a %d' % (old_volume, new_volume), reply=True, delete_after=20)

        else:
            if relative:
                raise exceptions.CommandError(
                    'No te pases de verga we. Pon un volumen entre {} y {:+}.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
            else:
                raise exceptions.CommandError(
                    'No te pases de verga we. Pon un volumen entre 1 y 100.'.format(new_volume), expire_in=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        lines = []
        unlisted = 0
        andmoretext = '* ... y %s mas*' % ('x' * len(player.playlist.entries))
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Reproduciendo: **%s** añadida por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Reproduciendo: **%s** %s\n" % (player.current_entry.title, prog_str))

        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = '`{}.` **{}** añadida por **{}**'.format(i, item.title, item.meta['author'].name).strip()
            else:
        ...
        if not lines:
            lines.append(
                'No hay canciones en la lista we! Pon algunas con {}play.'.format(self.config.command_prefix))
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        if not info.get('entries', None):
            ...
            if info.get('url', None) != info.get('webpage_url', info.get('url', None)):
                raise exceptions.CommandError("Ese link esta mal we.", expire_in=25)
            else:
        ...
        if not exfunc:
            raise exceptions.CommandError("Disculpa, no puedo reproducir links de ahi.", expire_in=25)
